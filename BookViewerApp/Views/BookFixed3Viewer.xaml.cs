...
public sealed partial class BookFixed3Viewer : Page
{
    ...
    private void flipView_ContextRequested(UIElement sender, ContextRequestedEventArgs args)
    {
        ...
        if (menuFlyout.Items.FirstOrDefault(a => a.Tag?.ToString() == "Bookmark") is MenuFlyoutSubItem menuBookmark && Binding is not null)
        {
            ...
            //    var newMenu = new MenuFlyoutSubItem() { Text = menuBookmark.Text, FlowDirection = menuBookmark.FlowDirection, Icon = menuBookmark.Icon, Tag = menuBookmark.Tag };
            //    foreach (var item in menuBookmark.Items)
            //    {
            //        newMenu.Items.Add(item);
            //    }
            //    menuFlyout.Items.Insert(index, newMenu);
            //}
        }
        //var index = menuFlyout.Items.IndexOf(menuFlyout.Items.First(a => a.Tag?.ToString() == "Insert1"));
        //menuFlyout.Items.Insert(index, new MenuFlyoutItem() { Text = "Bookmark" });
        //menuFlyout.Items.Insert(index, new MenuFlyoutSeparator());
