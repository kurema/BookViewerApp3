...
namespace BookViewerApp.Views
{
	...
	public sealed partial class SettingPage : Page, IThemeChangedListener
	{
		...
		private async void SettingPage_Loaded(object sender, RoutedEventArgs e)
		{
			...
			var purchaseList = (await GetPurchaseItems()).ToList();
			{
				var tabpage = UIHelper.GetCurrentTabPage(this);
				var loader = Managers.ResourceManager.Loader;
				purchaseList.Add(new ViewModels.ListItemViewModel(loader.GetString("Info/Info/BeSponsor/Title"), loader.GetString("Info/Info/BeSponsor/Description"), new OpenWebCommand(tabpage, "https://github.com/sponsors/kurema/")) { GroupTag = "Info/Purchase/Title" });
			}
			if (purchaseList.Count() >= 1) collection.Insert(Math.Max(collection.Count - 1, 0), purchaseList.GroupBy(a => Managers.ResourceManager.Loader.GetString(a.GroupTag)).First());
		...
		public class SettingViewModel : INotifyPropertyChanged
		{
			...
			public string CustomUIKey => target.CustomUIKey;
	...
	{
		public class SettingTemplateSelectorCustomTemplate
		{
		...
		public sealed class SettingTemplateSelector : DataTemplateSelector
		{
			...
			public ObservableCollection<SettingTemplateSelectorCustomTemplate> TemplateCustoms { get; } = new();
			...
			protected override DataTemplate SelectTemplateCore(object item, DependencyObject container)
			{
				...
				if (!string.IsNullOrEmpty(itemVM.CustomUIKey))
				{
					var template = TemplateCustoms?.FirstOrDefault(a => a.Key == itemVM.CustomUIKey);
					if (template is not null) return template.Template;
				}
