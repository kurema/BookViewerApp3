...
public static class HistoryManager
{
	...
	public static string AddEntry(IStorageItem file, string ID = null)
	{
		if (file is null) return null;
		var metadata = new Metadata() { Name = file.Name, ID = ID ?? string.Empty, Date = DateTimeOffset.Now }.Serialize();
		string token = List.Add(file, metadata);
		OnUpdated();
		return token;
	}
